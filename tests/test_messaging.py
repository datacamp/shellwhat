...
@pytest.mark.parametrize('fixed, patt', [
    (True, 'The checker expected to find `not_in_result` in the output of your command.'),
    (False, 'The checker expected to find the pattern `not_in_result` in the output of your command.')
])
...
def test_has_cwd(state):
    with pytest.raises(TF, match='Your current working directory should be `not_right_dir`. Use `cd not_right_dir` to navigate there.'):
